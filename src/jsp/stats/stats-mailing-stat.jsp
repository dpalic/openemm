<%@ page language="java" contentType="text/html; charset=utf-8"
         import="org.agnitas.beans.TrackableLink, org.agnitas.stat.MailingStatEntry, org.agnitas.stat.URLStatEntry, org.agnitas.target.Target, org.agnitas.util.AgnUtils, org.agnitas.util.EmmCalendar" %>
...
<%
    ...
    Integer maxblue = (Integer) request.getAttribute("maxblue");
    Integer maxNRblue = (Integer) request.getAttribute("maxNRblue");
    Integer maxSubscribers = (Integer) request.getAttribute("maxSubscribers");
...
<html:link page='<%= \"/ecs_stat.do?mailingId=\" + tmpMailingID + \"&init=true\" %>'><html:img src="button?msg=ecs.Heatmap"
                                                                                               border="0"/></html:link>
...
<table border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <td valign="bottom">
            ...
            <html:image align="bottom" src="button?msg=button.Add" border="0" property="add" value="add"/><br>&nbsp;
        </td>
        <% } %>
        <td align="right">
...
<tr>
    ...
    <td align="right">&nbsp;<b><%=aktMailingStatEntry.getTargetName()%></b>
                ...
                <% if(targets.size()>1) { %>
                &nbsp;<html:link page='<%= new String(\"/mailing_stat.do?action=\" + Integer.toString(MailingStatAction.ACTION_MAILINGSTAT) + \"&delTargetID=\" + aktTargetID) %>'><img src="${emmLayoutBase.imagesURL}/delete.gif" alt="<bean:message key='button.Delete'/>" border="0"></html:link>&nbsp;
                <% } %>
            </td>
...
<% /* * * * * * * * * * * * * */ %>
<tr>
    <td><bean:message key="statistic.ClicksBruttoNetto"/>:&nbsp;</td>
    <% file += "\"" + SafeString.getLocaleString("statistic.ClicksBruttoNetto", (Locale) request.getSession().getAttribute(org.apache.struts.Globals.LOCALE_KEY)) + "\";\"\"";
        targetIter = targets.listIterator();
        while (targetIter.hasNext()) {
            aktTargetID = ((Integer) targetIter.next()).intValue();
            MailingStatEntry aktMailingStatEntry = (MailingStatEntry) statValues.get(new Integer(aktTargetID));
...
<tr class="stats_normal_color">
    <td>&nbsp;<html:link
            ...
            key="Opened_Mails"/>:</B></html:link>&nbsp;</td>
    <%
