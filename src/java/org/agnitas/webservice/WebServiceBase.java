...
public class WebServiceBase extends ServletEndpointSupport {
    ...
    protected Hashtable usedConnections=null;
    ...
    protected Connection getConnection(MessageContext msct) {
        ...
        if(dbConn!=null) {
            if(this.usedConnections==null) {
                if(aServlet.getServletContext().getAttribute("usedConnections")==null) {
                    usedConnections=new Hashtable();
                    aServlet.getServletContext().setAttribute("usedConnections", usedConnections);
                } else {
                    usedConnections=(Hashtable)aServlet.getServletContext().getAttribute("usedConnections");
                }
            }
            try {
                ...
                this.usedConnections.put(dbConn, "<b>"+e.getMessage()+"</b><br>"+aSWriter.getBuffer());
            }
        }
    ...
    protected void freeConnection(Connection dbConn) {
        if(dbConn!=null) {
            this.usedConnections.remove(dbConn);
            try {
                dbConn.close();
            } catch(Exception e) {
                // do nothing
